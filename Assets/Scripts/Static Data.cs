...
public class StaticData : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        applause_audio= GameObject.Find("Applause Audio");
        notification_on_audio= GameObject.Find("Notification ON");
    ...
    void Update()
    {
        ...
        GameObject[] Game_Audio_list = {seller_voice,monkey_voice,collected_audio,dropped_audio,applause_audio,notification_on_audio,notification_off_audio};
        ...
        if(currentScene!=previousScene){
                //Audio off
                for(int i = 0; i <Panel_Audio_list.Length;i++){
                    AudioSource audioSource = Panel_Audio_list[i].GetComponent<AudioSource>();
                    audioSource.enabled = false;
                }
                for(int i = 0; i <Game_Audio_list.Length;i++){
                    AudioSource audioSource = Game_Audio_list[i].GetComponent<AudioSource>();
                    audioSource.enabled = false;
                }
                previousScene =currentScene;
        }
